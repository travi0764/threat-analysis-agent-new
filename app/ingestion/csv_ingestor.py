...
from app.storage.models import IndicatorType, SourceType, Indicator
            ...
            for idx, row in enumerate(rows, start=1):
                ...
                try:
                    ...
                    if self.skip_duplicates:
                        existing = db_session.query(Indicator).filter_by(
                        ...
                        if existing:
                            ...
                            logger.debug(f"Updated existing indicator: {indicator_data.value}")
                            continue
                    ...
                    logger.debug(f"Created indicator: {indicator.value} ({indicator.indicator_type.value})")
    ...
    def normalize_indicator(self, raw_indicator: Dict[str, Any]) -> Optional[IndicatorData]:
        ...
        try:
            ...
            if "indicator_type" in raw_indicator and raw_indicator["indicator_type"]:
                try:
                    indicator_type = IndicatorType(raw_indicator["indicator_type"].lower())
                except ValueError:
                    if self.auto_detect_type:
                        indicator_type = detect_indicator_type(value)
                    else:
                        raise ValidationError(f"Invalid indicator_type: {raw_indicator['indicator_type']}")
            else:
                if self.auto_detect_type:
                    indicator_type = detect_indicator_type(value)
                else:
                    raise ValidationError("indicator_type is required when auto-detect is disabled")
