...
from app.storage.repository import FeedbackRepository, ClassificationRepository, IndicatorRepository
...
class FeedbackRequest(BaseModel):
    ...
    corrected_risk_level: Optional[str] = Field(None, description="Corrected risk level if incorrect")
    ...
    # allow a clear FP/FN decision, record as UNCERTAIN.
        ...
        if feedback_type_str == "correct":
            ...
            if corrected_level is not None:
                ...
                if original_level in [RiskLevel.HIGH, RiskLevel.MEDIUM] and corrected_level == RiskLevel.LOW:
                    feedback_type = FeedbackType.FALSE_POSITIVE
                elif original_level == RiskLevel.LOW and corrected_level in [RiskLevel.HIGH, RiskLevel.MEDIUM]:
...
async def get_feedback_stats(db: Session = Depends(get_db_session)):
    ...
    try:
        ...
        incorrect_count = counts.get('false_positive', 0) + counts.get('false_negative', 0) + counts.get('uncertain', 0)
...
async def get_indicator_feedback(
    ...
    try:
        ...
        return {
            ...
            "original_risk_level": feedback.original_risk_level.value if feedback.original_risk_level else None,
            "corrected_risk_level": feedback.corrected_risk_level.value if feedback.corrected_risk_level else None,
            "comment": feedback.comment,
