...
from app.storage.models import Indicator, IndicatorType, RiskLevel
...
class ClassificationOutput(BaseModel):
    ...
    confidence: float = Field(description="Confidence in classification from 0.0 to 1.0")
...
class ThreatAnalysisAgent:
    ...
    def _observe_whois(self, data: Dict[str, Any], score: float) -> List[str]:
        ...
        if creation_date:
            try:
                ...
                if age_days < 30:
                    observations.append(f"Domain is very new (created {age_days} days ago) - HIGH RISK")
                elif age_days < 90:
                    observations.append(f"Domain is new (created {age_days} days ago) - MEDIUM RISK")
                elif age_days < 365:
                    observations.append(f"Domain is recent (created {age_days} days ago)")
                else:
        ...
        if country:
            ...
            if country in high_risk_countries:
                observations.append(f"Registered in {country} - potentially high-risk region")
            else:
    ...
    def _observe_ip_reputation(self, data: Dict[str, Any], score: float) -> List[str]:
        ...
        if abuse_score >= 80:
            ...
            observations.append(f"Moderate abuse confidence ({abuse_score}%) - HIGH RISK")
        elif abuse_score >= 20:
    ...
    def _observe_hash_lookup(self, data: Dict[str, Any], score: float) -> List[str]:
        ...
        if detections > 0:
            ...
            if percentage >= 50:
                observations.append(f"HIGH malware detection ({detection_ratio}) - CRITICAL")
            elif percentage >= 20:
                observations.append(f"MODERATE malware detection ({detection_ratio}) - HIGH RISK")
            else:
    ...
    def _reason_node(self, state: AgentState) -> Dict[str, Any]:
        ...
        user_prompt = self._build_user_prompt(indicator, observations, avg_score, max_score)
        ...
        try:
            ...
            classification = classification_data if isinstance(classification_data, dict) else (classification_data.model_dump() if hasattr(classification_data, 'model_dump') else classification_data.dict())
    ...
    def _build_system_prompt(self) -> str:
        ...
        return f"""You are an expert cybersecurity analyst specializing in threat intelligence analysis.
