...
class MockWhoisEnricher(BaseEnricher):
    ...
    async def enrich(
        ...
        # Generate mock data
        registrars = ["GoDaddy", "Namecheap", "CloudFlare", "Google Domains", "Network Solutions"]
        countries = ["US", "CN", "RU", "BR", "IN", "DE", "FR"]
        
        # Suspicious indicators get older creation dates and certain registrars
        is_suspicious = any(word in indicator_value.lower() for word in 
                          ["evil", "phish", "malware", "hack", "scam", "fake", "bad"])
        ...
        data = {
            ...
            "updated_date": (datetime.utcnow() - timedelta(days=random.randint(1, 365))).isoformat(),
...
class MockIPReputationEnricher(BaseEnricher):
    ...
    async def enrich(
        ...
        data = {
            ...
            "isp": random.choice(["AWS", "Google Cloud", "DigitalOcean", "OVH", "Hetzner"]),
