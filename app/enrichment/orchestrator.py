...
from app.storage.models import Indicator, IndicatorType
...
class EnrichmentOrchestrator:
    ...
    async def enrich_indicators_batch(
        ...
        for indicator, result in zip(indicators, results):
            if isinstance(result, Exception):
                logger.error(f"Batch enrichment failed for indicator {indicator.id}: {result}")
    ...
    def get_enrichment_summary(
        ...
        if enrichments:
            ...
            summary["enrichment_types"] = list(set(e.enrichment_type for e in enrichments))
